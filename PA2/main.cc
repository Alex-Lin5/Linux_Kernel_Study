...
main(int argc, char **argv)
{
    int i;
    char *debugArg = "";
    List<char *>* userProgNames = new List<char *>();
    ...
    */
    int idx = 0;
    for (ListIterator<char *>* itr = new ListIterator<char *>(userProgNames);
		    !itr->IsDone(); itr->Next()){
	// RunUserProg(itr->Item());
	/*
	if (itr->Item() == userProgNames->Front()){
	    int idx = 0;
	} else{
	    idx++;
	}
	*/
    ...
    // deallocate the userProgName list
    while (userProgNames->NumInList() > 0){
    	userProgNames->RemoveFront();
    }
    delete userProgNames;
